...
void solve(){
	...
	read(P);
	read(Q);
	ll terms = D/d;		// Total combinations formed
	ll rem_days = D%d;	
	// Remaining days which are not part of any complete combination

	ll sum1 = (P * d * terms) + Q * terms * (terms - 1) * d/2;
	ll sum2 = sum1 + rem_days * (P + terms * Q);
	cout<<sum2<<"\n"; 
}
