...
void solve(){
	...
	reads(s);
	ll ans = 0;
	ll currEmptyStreak = 0;
	ll maxEmptyStreak = 0;
	ll totalEmptyCells = 0;

	s.pb('#');

	rep(i, 0, n + 1) {
		if(s[i] == '.') currEmptyStreak++;
		else {
			maxEmptyStreak = max(maxEmptyStreak, currEmptyStreak);
			totalEmptyCells += currEmptyStreak;
			currEmptyStreak = 0;
		}
	}
	if(maxEmptyStreak >= 3) cout << 2 << "\n";
	else cout << totalEmptyCells << "\n";
}
